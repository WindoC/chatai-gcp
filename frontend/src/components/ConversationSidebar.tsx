...
export const ConversationSidebar: React.FC<ConversationSidebarProps> = ({
  ...
  const ConversationItem: React.FC<{ conversation: ConversationSummary }> = ({ conversation }) => (
    <div
      className={`
        ...
        ${conversation.conversation_id === currentConversationId ? 'bg-gray-700' : ''}
      `}
    ...
    >
      <div className="flex-1 min-w-0">
        ...
        <p className="text-gray-400 text-xs mt-1">{formatDate(conversation.last_updated)}</p>
        {conversation.preview && (
          <p className="text-gray-500 text-xs mt-1 truncate">{conversation.preview}</p>
        )}
      </div>
      ...
      <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        >
          <svg className={`w-4 h-4 ${conversation.starred ? 'text-yellow-400' : 'text-gray-400'}`} fill="currentColor" viewBox="0 0 20 20">
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
          </svg>
        </button>
        <button
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`
        fixed md:relative inset-y-0 left-0 z-40 w-80 bg-gray-800 
      ...
      `}>
        <div className="flex flex-col h-full">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            {loading ? (
              <div className="text-gray-400 text-center">Loading conversations...</div>
            ) : (
              <>
                {/* Starred Conversations */}
                {starredConversations.length > 0 && (
                  <div>
                    <h3 className="text-gray-400 text-xs uppercase font-semibold mb-2">Starred</h3>
                ...
                {unstarredConversations.length > 0 && (
                  <div>
                    <h3 className="text-gray-400 text-xs uppercase font-semibold mb-2">Recent</h3>
                    <div className="space-y-1">
      ...
      {showBulkDeleteConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg max-w-md w-mx-4">
